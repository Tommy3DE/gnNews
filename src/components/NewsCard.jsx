...
const NewsCard = ({ handleToggle, article, engVersion }) => {
  return (
    <div className=" animate-fade-in z-10 bg-gradient-to-b from-black w-full top-0 left-0 h-screen pt-10 fixed">
      <div className="bg-white md:w-3/5 w-11/12 rounded flex lg:flex-row flex-col border-2 border-gray-300 fixed md:left-[20%] md:top-[15%] left-[4%]">
        ...
        <div className="flex flex-col xl:justify-around md:justify-center mx-3 w-auto">
          <h3 className="2xl:text-3xl lg:text-xl md:text-lg w-full">{article.title ? article.title : 'News'} </h3>
          <h2 className="2xl:text-xl lg:text-lg md:text-lg text-gray-700">{article.author ? article.author : `${engVersion? 'Author not known' : 'Autor nie znany'}`}</h2>
          <h1 className="2xl:text-lg lg:text-md md:text-sm text-gray-400">{article.content ? article.content.replace(/\s*\[.+?\]/g, '') : ''}</h1>
